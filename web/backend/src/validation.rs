...
pub fn parse_signin_info(body: &[u8]) -> Option<(String, String)> {
    let equ1 = web_utils::find_byte(body, b'=')?;
    let key1 = &body[..equ1];

    if key1 != b"username" { return None; }

    let body     = &body[equ1+1..];
    let amp1     = web_utils::find_byte(body, b'&')?;
    let value1   = &body[..amp1];
    let username = web_utils::undo_urlencoding(value1)?;
    ...
    let equ2 = web_utils::find_byte(body, b'=')?;
    let key2 = &body[..equ2];

    if key2 != b"password" { return None; }

    let value2   = &body[equ2+1..];
    let password = web_utils::undo_urlencoding(value2)?;
